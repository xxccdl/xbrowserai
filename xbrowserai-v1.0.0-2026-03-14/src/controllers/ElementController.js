class ElementController {
  ...
  async clickByText(text, tagName = '*') {
    try {
      const element = await this.page.evaluateHandle((searchText, tag) => {
        ...
        const matchElement = (el) => {
          ...
          if (elText === searchText) return 4;
          if (elTextLower === searchLower) return 3;
          if (elText.includes(searchText)) return 2;
          if (elTextLower.includes(searchLower)) return 1;
          return 0;
        };
        
        for (const priorityTag of priorityTags) {
          const selector = tag === '*' ? priorityTag : `${tag}${priorityTag.includes('[') ? '' : ',' + priorityTag}`;
